...
//   // 1. Get current stock from inventory (sum of all warehouses)
//   const { data: stockInventoryRows, error: stockInventoryErr } = await supabase
//     .from("inventory")
//     .select("quantity")
//     .eq("product_id", productId);

//   if (stockInventoryErr) {
//     return NextResponse.json(
//       error(stockInventoryErr.message || "Failed to fetch stock", 500),
//       {
//         status: 500,
//       }
//     );
...
//   // 2. Fetch min_stock from product
...
//   // 3. Fetch last stock movement (manual join)
//   const { data: stockTxRaw, error: stockTxRawError } = await supabase
...
//     return NextResponse.json(
//       error(
//         stockTxRawError.message || "Failed to fetch stock transaction",
//         500
//       ),
//       {
//         status: 500,
...
//   if (stockTxRaw) {
//     // fetch invoice item to get invoice_id
...
//   if (poError || !poItems) {
//     return NextResponse.json(
//       error("Failed to retrieve purchase order items", 500),
//       { status: 500 }
//     );
//   }
...
//   // 4. Fetch purchase_orders
//   const { data: purchaseOrders, error: poDetailsError } = await supabase
//     .from("purchase_order")
//     .select(
...
//   if (poDetailsError || !purchaseOrders) {
//     return NextResponse.json(error("Failed to load POs", 500), {
//       status: 500,
//     });
//   }
...
//     .select("id, name")
//     .in("id", supplierIds);
//   const suppliersTyped = (suppliers ?? []) as { id: number; name: string }[];
...
//     .in("id", currencyIds);
//   const currenciesTyped = (currencies ?? []) as {
//     id: number;
//     currency_code: string;
//   }[];

//   // 7. Transform into usage history
//   const purchase_orders: ProductPurchaseOrder[] = poItems
//     .map((item) => {
...
//       const currencyCode =
//         currenciesTyped.find((c) => c.id === po.currency_id)?.currency_code ||
//         "THB";

//       const supplierName =
//         suppliersTyped.find((s) => s.id === po.supplier_id)?.name || "Unknown";
...
//   const paginatedOrders = purchase_orders.slice(from, to + 1);
...
//     purchase_orders: paginatedOrders,
//     page,
//     pageSize,
